...
if [ -d "${APP_NAME}" ]; then
  ...
  exec_cmd "git checkout ${APP_GIT_BRANCH}" "Checkout to ${APP_GIT_BRANCH}"
  exec_cmd "git pull" "Fetch and integrate changes from ${APP_GIT_URI}"
else
  exec_cmd "git clone --branch ${APP_GIT_BRANCH} ${APP_GIT_URI}" \
            "Clone application ${APP_NAME} from ${APP_GIT_URI} and checkout to ${APP_GIT_BRANCH} branch"
  exec_cmd "cd ${APP_NAME}" "Enter to ${APP_NAME} application directory"
fi

exec_cmd "bundle install" "Install the gems specified by the Gemfile"
...
while [ "${WEB_SERVER_LAUNCH_RETRIES}" -gt 0 ]; do
  if simple_http_check "http://${APP_BIND_ADDRESS}:${APP_BIND_PORT}"; then
    ...
    exec_cmd "puma --daemon" "Start puma server in daemon mode on ${APP_BIND_ADDRESS}:${APP_BIND_PORT}"
